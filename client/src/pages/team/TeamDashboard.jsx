...
import { getMyPhotos } from "../../services/photo.service";
...
function TeamDashboard() {
  ...
  const loadEvents = async () => {
    try {
      ...
      const eventsWithPhotoCounts =
        await Promise.all(
          assignedEvents.map(async (event) => {
            try {
              const photos =
                await getMyPhotos(event._id);
              ...
              console.error(
                `Failed to load photos for event ${event._id}:`,
                photoError
              );
          ...
          <div className="flex flex-col gap-8 md:flex-row md:items-end md:justify-between">
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              <button
                ...
                className="rounded-xl border border-slate-200 bg-white px-5 py-3 text-sm font-semibold text-slate-600 transition hover:bg-slate-50 hover:text-slate-900"
              >
                Logout
              </button>
            </div>
